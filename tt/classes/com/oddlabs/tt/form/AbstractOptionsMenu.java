...
//		addChild(language_list_box);
		...
		// Changing color mid-game loses player's ability to play. It's okay that it only can be changed in main menu.
        if (!(this instanceof InGameOptionsMenu)) {
			Group group_player_colors = CreatePlayerColorGroup();
        	general.addChild(group_player_colors);
			group_player_colors.place(group_aggressive_units, BOTTOM_LEFT);
        }
    ...
    private final Group CreatePlayerColorGroup() {
        ...
        Label[] playerLabels = new Label[Player.COLORS.length];
        EditLine[] hexboxes = new EditLine[Player.COLORS.length];
        // The first row label is the reference for placing the label on the next row
        for (int i = 0; i < Player.COLORS.length; i++) {
            Label colorLabel = new Label("Color " + (i + 1), Skin.getSkin().getEditFont(), 120);
            colorLabel.setColor(Player.COLORS[i]);
            ...
            // Splits the colors ui into two rows
            if (i < Player.COLORS.length / 2) {
                // first row is placed without any parameters
                if (i == 0) {
                    colorLabel.place();
                } else {
                    colorLabel.place(playerLabels[i - 1], RIGHT_TOP);
                }
                hexBox.place(colorLabel, BOTTOM_LEFT);
            } else {
                // place label below the color above it (i.e. player 1 + player 4, player 2 + player 5, ...)
                colorLabel.place(hexboxes[i - Player.COLORS.length / 2], BOTTOM_LEFT);
                hexBox.place(colorLabel, BOTTOM_LEFT);
            }

            group_player_colors.addChild(colorLabel);
            group_player_colors.addChild(hexBox);
        }

        // TODO: Localization
        HorizButton update_colors_btn = new HorizButton("Update", 130);
        update_colors_btn.addMouseClickListener(new UpdateColorsListener(playerLabels, hexboxes));
        group_player_colors.addChild(update_colors_btn);
        update_colors_btn.place(hexboxes[hexboxes.length - 1], BOTTOM_LEFT);

        HorizButton reset_colors_btn = new HorizButton("Reset", 130);
        reset_colors_btn.addMouseClickListener(new ResetColorsListener(playerLabels, hexboxes));
        group_player_colors.addChild(reset_colors_btn);
        reset_colors_btn.place(update_colors_btn, LEFT_MID);

        group_player_colors.compileCanvas();
        return group_player_colors;
    }
    ...
    private final strictfp class ResetColorsListener implements MouseClickListener {

        private final Label[] playerLabels;
        private final EditLine[] hexboxes;

        public ResetColorsListener(Label[] playerLabels, EditLine[] hexboxes) {
            super();
            this.playerLabels = playerLabels;
            this.hexboxes = hexboxes;
        }

        public final void mouseClicked(int button, int x, int y, int clicks) {
            Player.COLORS = Player.DEFAULT_COLORS;
            for (int i = 0; i < Player.COLORS.length; i++) {
                java.awt.Color c = new java.awt.Color(Player.COLORS[i][0], Player.COLORS[i][1], Player.COLORS[i][2], Player.COLORS[i][3]);
                String hex = String.format("#%02X%02X%02X", c.getRed(), c.getGreen(), c.getBlue());
                hexboxes[i].set(hex);
                playerLabels[i].setColor(Player.COLORS[i]);
                switch (i) {
                    case 0:
                        Settings.getSettings().player_color_1 = hex;
                        break;
                    case 1:
                        Settings.getSettings().player_color_2 = hex;
                        break;
                    case 2:
                        Settings.getSettings().player_color_3 = hex;
                        break;
                    case 3:
                        Settings.getSettings().player_color_4 = hex;
                        break;
                    case 4:
                        Settings.getSettings().player_color_5 = hex;
                        break;
                    case 5:
                        Settings.getSettings().player_color_6 = hex;
                        break;
                    default:
                        break;
                }
    ...
    private final strictfp class UpdateColorsListener implements MouseClickListener {

        private final Label[] playerLabels;
        private final EditLine[] hexboxes;

        public UpdateColorsListener(Label[] playerLabels, EditLine[] hexboxes) {
            super();
            this.playerLabels = playerLabels;
            this.hexboxes = hexboxes;
        }
        ...
        public final void mouseClicked(int button, int x, int y, int clicks) {
            for (int i = 0; i < Player.COLORS.length; i++) {
                String hex = hexboxes[i].getContents();
                java.awt.Color c = java.awt.Color.decode(hex);
                Player.COLORS[i] = new float[]{c.getRed() / 255f, c.getGreen() / 255f, c.getBlue() / 255f, c.getAlpha() / 255f};
                playerLabels[i].setColor(Player.COLORS[i]);
                switch (i) {
                    case 0:
                        Settings.getSettings().player_color_1 = hex;
